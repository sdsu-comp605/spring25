...
function mygemm_packed_MT2!(C, A, B, sizes::Sizes)
  ...
  mygemm_JPI_JI_packed_MT2!(C, A, B, A_pack, B_pack, sizes)
end

function mygemm_JPI_JI_packed_MT2!(C, A, B, A_pack, B_pack, sizes::Sizes)
  n = size(C, 2)
  ...
  @inbounds for j1 = 1:nc:n
    ...
    C_J = @view C[:, j1:j2]
    B_J = @view B[:, j1:j2]
    mygemm_PI_JI_packed_MT2!(C_J, A, B_J, A_pack, B_pack, sizes)
  end
end
...
@inline function mygemm_PI_JI_packed_MT2!(C, A, B, A_pack, B_pack, sizes::Sizes)
  k = size(A, 2)
  ...
  @inbounds for p1 = 1:kc:k
    ...
    mygemm_I_JI_packed_MT2!(C, A_P, A_pack, B_pack, kc_nr, kc_mr, sizes)
  end
end
...
@inline function mygemm_I_JI_packed_MT2!(C, A, A_pack, B_pack, kc_nr, kc_mr, sizes::Sizes)
  m = size(C, 1)
  ...
  @inbounds for i1 = 1:mc:m
    ...
    mygemm_JI_packed_MT2!(C_I, A_pack, B_pack, kc_nr, kc_mr, sizes)
  end
end

@inline function mygemm_JI_packed_MT2!(C, A_pack, B_pack, kc_nr, kc_mr, sizes::Sizes)
  n = size(C, 2)

  nr = sizes.nr
  @inbounds Threads.@threads for t = 1:cld(n, nr)
    j1 = (t - 1) * nr + 1
    j2 = min(n, j1 + nr - 1)
    C_J = @view C[:, j1:j2]
    tile_rng = (1 + kc_nr * (t - 1)) : (t * kc_nr)
    B_pack_J = @view B_pack[tile_rng]
    mygemm_I_packed_MT2!(C_J, A_pack, B_pack_J, kc_mr, sizes)
  end
end

@inline function mygemm_I_packed_MT2!(C, A_pack, B_pack, kc_mr, sizes::Sizes)
  m = size(C, 1)
  ...
  @inbounds for (t, i1) = enumerate(1:mr:m)
    ...
    microknl_packed_MT2!(C_I, A_pack_I, B_pack, sizes)
  end
end

@inline function microknl_packed_MT2!(C, A, B, sizes::Sizes)
  mr = sizes.mr
  nr = sizes.nr
  vl = sizes.vl
