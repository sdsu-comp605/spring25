...
let
  ...
  if isodd(mpirank)
    # Args: message, whom to, tag, communicator
    MPI.Recv!(recvmsg, recvfrom, 777, mpicomm)

    # Args: message, whom to, tag, communicator
    MPI.Send(sendmsg, sendto, 777, mpicomm)
  else
    # Args: message, whom to, tag, communicator
    MPI.Send(sendmsg, sendto, 777, mpicomm)
  ...
  print("$mpirank sent $sendmsg to $sendto and received $recvmsg from $recvfrom \n")
end
